...
const publishingDate = '2022-06-06';
const publishingDate1 = '2022-05-30';
...
describe('API e2e', () => {
  describe('Propositions routes', () => {
    ...
    it('Should book a slot, logged as user, with no proposition pending', async () => {
      const response = await userSession.put(`/v1/propositions/book/${publishingDate}`);
      expect(response.status).toBe(201);
    });
    it('Should get a 400, because this slot is already taken', async () => {
      const response = await userSession.put(`/v1/propositions/book/${publishingDate1}`);
      expect(response.status).toBe(400);
    });
    afterAll(async () => {
      const response = await adminSession.put(`/v1/propositions/unbook/${publishingDate}`);
      expect(response.status).toBe(201);
    });
  });
});
