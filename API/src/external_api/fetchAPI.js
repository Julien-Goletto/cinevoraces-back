...
const fetchAPI = {
   ...
   * @param {searchQuery} String : query from user
   * @param {TMDB_API_KEY} String : user key for API
   * @returns {Array} Movie objects with properties (title, original_title, genres, language, release, runtime, countries, languages, poster, directors, actors)
   */
  async fetchTop5ResultsWithDetails(searchQuery,TMDB_API_KEY){
    ...
    for (const movie of movies){
      const detailledMovie = await fetchAPI.getMovieDetails(movie,TMDB_API_KEY);
   ...
   * @param {TMDB_API_KEY} String : user key for API
   * @returns {movieWithDetails} Movie object with properties : (title, original_title, genres, language, release, runtime, countries, languages, poster, directors, actors)
   */
